...
public class DiGraph<T extends Comparable<T>>{
    ...
    private List<String> getShortestPath(Vertex target){
            ...
            for (Vertex v = target; v !=null; v = v.previous){
                if(v.minDistance != 0)
                    path.add("Ke "+ANSI_GREEN+v.value +ANSI_RESET+ " : total cost : " + v.minDistance);
                else
                    path.add("");
            }
            Vertex v = target;
    ...
    class Edge
    {
            ...
            public String toString()
            {
                    return "Edge From: " +ANSI_GREEN+ from.value +ANSI_RESET+ " to: "+ANSI_GREEN + to.value +ANSI_RESET+ " cost: " + cost;
            }
    }
}